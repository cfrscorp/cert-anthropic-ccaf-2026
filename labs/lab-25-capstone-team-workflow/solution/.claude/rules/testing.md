...
which directory it lives in. Because test files sit next to the code they cover
(`Button.test.tsx` beside `Button.tsx`), a glob-scoped rule is more maintainable
than a per-directory `CLAUDE.md` (Sample Question 6).

- Use the project test runner with `describe` / `it`; one top-level `describe`
  per unit under test.
